...
class TestListStrategies:
    ...
    def test_contains_all_builtins(self):
        ...
        missing = BUILTINS - names
        assert not missing, f"缺少内置策略: {missing}"

    def test_builtin_count_matches(self):
        """内置策略数量必须精确匹配（防止悄悄增减）"""
...
class TestRegisterCustom:
    ...
    def test_self_reference_raises(self):
        """base_strategy 与 name 相同时必须报错，防止无限递归"""
        from src.strategy.registry import register_from_config
        with pytest.raises(ValueError, match="自引用"):
            register_from_config("foo", "foo", {})

    def test_unknown_base_strategy_raises(self):
        """base_strategy 不存在时必须报错"""
        from src.strategy.registry import register_from_config
        with pytest.raises(ValueError, match="未知基础策略"):
            register_from_config("bar", "nonexistent_strategy_xyz", {})
