...
def register_from_config(
    ...
    """
    if base_strategy == name:
        raise ValueError(
            f"自引用错误: base_strategy('{base_strategy}') 不能与注册名('{name}')相同，"
            f"请设置 base_strategy 为一个已存在的内置策略（如 'mcts'、'greedy'）"
        )
    if base_strategy not in _REGISTRY:
        raise ValueError(
            f"未知基础策略: '{base_strategy}'，可用: {list(_REGISTRY.keys())}"
        )
...
def register_from_manifest(manifest_path: str) -> str:
    ...
    base = manifest.base_strategy or manifest.name
